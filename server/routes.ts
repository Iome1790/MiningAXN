...
🤖 Bot: @${_admBotName}`;
  ...
  // ─── TURBO RATING SYSTEM ──────────────────────────────────────────────────
  app.get("/api/turbo-rating", authenticateTelegram, async (req: any, res) => {
    try {
      const user = req.user?.user;
      if (!user) return res.status(401).json({ message: "Not authenticated" });
      ...
      const turboRating = parseFloat(String(fullUser.turboRating ?? "0"));
      const turboAdsWatched = fullUser.turboAdsWatched ?? 0;
      const firstWithdrawDone = fullUser.firstWithdrawDone ?? false;
      const lastTurboAdAt = (fullUser as any).lastTurboAdAt;

      // Cooldown: 30 seconds between ads
      const COOLDOWN_MS = 30_000;
      let canWatchAd = true;
      let nextAdAvailableIn = 0;
      if (lastTurboAdAt) {
        const elapsed = Date.now() - new Date(lastTurboAdAt).getTime();
        if (elapsed < COOLDOWN_MS) {
          canWatchAd = false;
          nextAdAvailableIn = Math.ceil((COOLDOWN_MS - elapsed) / 1000);
        }
      }
      ...
      console.error("Turbo rating GET error:", error);
      return res.status(500).json({ message: "Internal server error" });
    }
  });

  app.post("/api/turbo-rating/watch-ad", authenticateTelegram, async (req: any, res) => {
    try {
      const user = req.user?.user;
      if (!user) return res.status(401).json({ message: "Not authenticated" });
      ...
      if (lastTurboAdAt) {
        ...
        if (elapsed < COOLDOWN_MS) {
          return res.status(429).json({
            message: `Please wait ${Math.ceil((COOLDOWN_MS - elapsed) / 1000)}s before watching another ad`,
          });
        }
      }

      const currentRating = parseFloat(String(fullUser.turboRating ?? "0"));
      const adsWatched = (fullUser.turboAdsWatched ?? 0) + 1;
      const firstWithdrawDone = fullUser.firstWithdrawDone ?? false;
      ...
      if (!firstWithdrawDone) {
        // First withdrawal boost sequence — fast progression
        const firstBoosts = [0.4, 1.2, 2.3, 6.1];
        const idx = adsWatched - 1;
        if (idx < firstBoosts.length) {
          boost = firstBoosts[idx];
        } else {
          // After sequence, smaller boosts capped at MAX_RATING
          boost = parseFloat((Math.random() * 0.4 + 0.3).toFixed(2));
        }
      } else {
        // Normal mode — slower, random boosts
        boost = parseFloat((Math.random() * 0.5 + 0.7).toFixed(2));
      }

      let newRating = Math.min(MAX_RATING, parseFloat((currentRating + boost).toFixed(4)));

      await db.execute(sql`
        UPDATE users
        SET
          turbo_rating = ${newRating.toString()},
          turbo_ads_watched = ${adsWatched},
          last_turbo_ad_at = NOW()
        WHERE id = ${user.id}
      `);
      ...
      return res.json({
        newRating,
        boost,
        turboAdsWatched: adsWatched,
        firstWithdrawDone,
      });
    } catch (error) {
      console.error("Turbo rating watch-ad error:", error);
      return res.status(500).json({ message: "Internal server error" });
    }
  });

  // When a turbo withdrawal is submitted, mark first_withdraw_done
  app.post("/api/turbo-rating/mark-first-withdraw", authenticateTelegram, async (req: any, res) => {
    try {
      const user = req.user?.user;
      if (!user) return res.status(401).json({ message: "Not authenticated" });
      await db.execute(sql`
        UPDATE users SET first_withdraw_done = true WHERE id = ${user.id}
      `);
      return res.json({ success: true });
    } catch (error) {
      console.error("Mark first withdraw error:", error);
      return res.status(500).json({ message: "Internal server error" });
    }
  });
  // ─────────────────────────────────────────────────────────────────────────
